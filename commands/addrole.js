...
module.exports.run = async (bot, message, args) => {
  ...
  if(!message.member.roles.some(role=>["ROLE NAME #1", "ROLE NAME #2", "ROLE NAME #3"].includes(role.name))) return errors.noPerms(message, ";addrole <user> <role>", "Try becoming an HR!");
  ...
  bot.guilds.get("GUILD ID").channels.get("CHANNEL ID").send(logEmbed); // DDS Logs
}
